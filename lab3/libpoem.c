...
// 	char value[4];
// };
// static uid_t (*old_id)(void) = NULL;
long got_list[sizeof(ndat)/sizeof(int)];
...
int init()   {
	...
	long value;
	ptr = fopen("got_index.txt", "r");
	while(fgets(buffer, buffersize, ptr)){
		int temp_key;
		sscanf(buffer, "%d %lx", &temp_key, &value);
		// printf("%d, %lx\n", temp_key, value);
		got_list[temp_key] = value;
	}
	fclose(ptr);
	for(int index = 0; index < sizeof(ndat)/sizeof(int); ++index){
		printf("%p, ", got_list[index]);
	}
	...
	for(int index = 0; index < (sizeof(ndat)/sizeof(int)); ++index){
		if(got_list[index] != 0){
			...
			// long realaddr = (base + got_list[index]);
...
// printf("%p, %p ,%p\n", chals_base, got_offset_min, (long*)got_page_ptr);
	...
	// if (mprotect(baseptr, page_size, PROT_READ | PROT_WRITE) == -1) {
	...
	// 	return 1;
	// }
	// long realptr = (chals_base + got_list[498]);
	// printf("%p, %p\n", (long*)realptr, old_list[498], old_list[44]);
	for(int index = 0; index < (sizeof(ndat)/sizeof(int)); ++index){
		if(got_list[index] != 0){
			printf("%d ", index);
			long realptr = (chals_base + got_list[index]);
			int old_index;
			for(old_index = 0; old_index < (sizeof(ndat)/sizeof(int)); old_index++){
				// printf("im here!");
				if (ndat[old_index] == index){
					break;
				} 
			}
			// if(index == 498)printf("%d\n", old_index);
			// printf("%p, %p\n", (long*)realptr, old_list[index]);
			*(long*)realptr = (long*)old_list[old_index];
			// memcpy(realptr, old_list[index], 8);
		}
	}
	// long realaddr = (chals_base + got_list[1]);
	// long *realptr = (long*)realaddr; 
	// printf("%ld, %ld, %ld\n", got_base, got_base + (got_offset_max - got_offset_min), realaddr);
	// *realptr = 0x11;
